...
void NMBoolSettingWidget::setValue(const SettingValue& value) {
  try {
    ...
    NOVELMIND_LOG_ERROR("Failed to set bool value for {}", m_definition.key);
  }
}
...
void NMIntSettingWidget::setValue(const SettingValue& value) {
  try {
    ...
    NOVELMIND_LOG_ERROR("Failed to set int value for {}", m_definition.key);
  }
}
...
NMFloatSettingWidget::NMFloatSettingWidget(const SettingDefinition& def,
  ...
  if (def.type == SettingType::FloatRange) {
    ...
    connect(m_slider, &QSlider::valueChanged, this, [this, def](int value) {
      f32 realValue = def.minValue + (value / 1000.0f) * (def.maxValue - def.minValue);
...
SettingValue NMFloatSettingWidget::getValue() const {
  if (m_slider) {
    ...
    return m_definition.minValue + (m_slider->value() / 1000.0f) * range;
  } else if (m_spinBox) {
...
void NMFloatSettingWidget::setValue(const SettingValue& value) {
  try {
    ...
    NOVELMIND_LOG_ERROR("Failed to set float value for {}", m_definition.key);
  }
}
...
void NMStringSettingWidget::setValue(const SettingValue& value) {
  try {
    ...
    NOVELMIND_LOG_ERROR("Failed to set string value for {}",
                        m_definition.key);
  }
}
...
void NMEnumSettingWidget::setValue(const SettingValue& value) {
  try {
    ...
    NOVELMIND_LOG_ERROR("Failed to set enum value for {}", m_definition.key);
  }
}
...
void NMPathSettingWidget::setValue(const SettingValue& value) {
  try {
    ...
    NOVELMIND_LOG_ERROR("Failed to set path value for {}", m_definition.key);
  }
}
...
void NMSettingsCategoryPage::applyValues() {
  for (auto* widget : m_widgets) {
    ...
    if (!error.empty()) {
      NOVELMIND_LOG_ERROR("Failed to set {}: {}", widget->getKey(), error);
    }
  }
}
...
NMSettingsCategoryPage::createWidgetForSetting(const SettingDefinition& def) {
  ...
  default:
    NOVELMIND_LOG_WARNING("Unsupported setting type for {}", def.key);
    return nullptr;
  }
}
...
void NMSettingsDialog::onResetClicked() {
  ...
  if (currentPage) {
    ...
    NOVELMIND_LOG_INFO("Category reset to defaults: {}",
