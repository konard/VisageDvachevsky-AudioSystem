...
TEST_CASE("ProjectJson - String escaping", "[project_json][escaping]") {
  ...
  metadata.name = "Test \"Project\" with special\nchars";
  ...
  REQUIRE(result.isOk());
  CHECK(json.find("\\\"") != std::string::npos);  // Escaped quotes
...
TEST_CASE("ProjectJson - Parse with escaped strings", "[project_json][parsing]") {
  ...
  REQUIRE(result.isOk());
  CHECK(metadata.name == "Project with \"quotes\"");
  CHECK(metadata.description == "Line 1\nLine 2\tTabbed");
}
