...
void createExampleDiagnostics() {
  // Example 1: Undefined variable with related info
  {
    ...
    diag.location = SourceLocation{
        .file = "main.script", .line = 42, .column = 10, .endLine = 42, .endColumn = 16};

    DiagnosticRelated related;
    related.location = SourceLocation{
        .file = "globals.script", .line = 5, .column = 1, .endLine = 5, .endColumn = 20};
    related.message = "Similar variable 'Player' defined here (note capitalization)";
    diag.relatedInfo.push_back(related);
  ...
  // Example 2: Deprecated function with suggestion
  {
    ...
    diag.location = SourceLocation{
        .file = "scene1.script", .line = 15, .column = 5, .endLine = 15, .endColumn = 30};

    DiagnosticFix fix;
    fix.title = "Replace with 'displayText()'";
    fix.description = "The new function provides better formatting options";
    fix.replacementText = "displayText(message, {style: 'default'})";
    fix.range = SourceLocation{
        .file = "scene1.script", .line = 15, .column = 5, .endLine = 15, .endColumn = 30};
  ...
  // Example 3: Cyclic dependency with multiple related and suggestions
  {
    ...
    diag.location = SourceLocation{
        .file = "chapter1.graph", .line = 25, .column = 1, .endLine = 25, .endColumn = 10};

    // Show all nodes involved in the cycle
    DiagnosticRelated rel1;
    rel1.location = SourceLocation{
        .file = "chapter1.graph", .line = 30, .column = 1, .endLine = 30, .endColumn = 10};
    rel1.message = "Node 'scene_A' points here";
    diag.relatedInfo.push_back(rel1);

    DiagnosticRelated rel2;
    rel2.location = SourceLocation{
        .file = "chapter1.graph", .line = 35, .column = 1, .endLine = 35, .endColumn = 10};
    ...
    ErrorReporter::instance().report(diag);
    std::cout << "Created diagnostic: Cyclic dependency with multiple related and "
                 "suggestions\n";
  }
  ...
  // Example 4: Missing asset with related info
  {
    ...
    diag.location = SourceLocation{
        .file = "scene_forest.scene", .line = 10, .column = 15, .endLine = 10, .endColumn = 35};

    DiagnosticRelated rel;
    rel.location = SourceLocation{
        .file = "scene_city.scene", .line = 8, .column = 15, .endLine = 8, .endColumn = 35};
    rel.message = "Asset also referenced here";
    diag.relatedInfo.push_back(rel);

    DiagnosticFix fix;
    fix.title = "Use placeholder texture";
    fix.description = "Replace with 'bg_placeholder.png' until the asset is available";
